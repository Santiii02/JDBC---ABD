...
public class Tests {
    ...
    public void ejecutarTestsAnularBilletes() {
        ...
        try {
            ...
            java.util.Date fecha = toDate("30/04/2024");
            int nroPlazas = 1;
            servicio.anularBillete(hora, fecha, ORIGEN, DESTINO, nroPlazas, ticketID);
            LOGGER.info("Anulación de billetes exitosa");
        } catch (CompraBilleteTrenException e) {
            LOGGER.error("Error durante la anulación del billete: " + e.getMessage());
    ...
    public void ejecutarTestsCompraBilletes() {
        ...
        try {
            ...
            if (e.getErrorCode() == CompraBilleteTrenException.NO_EXISTE_VIAJE) {
                LOGGER.info("Se ha capturado correctamente la excepción para un viaje inexistente.");
            }
        }
